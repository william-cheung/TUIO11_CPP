...
{   
  ...
  switch (remove_method)
  {
	  ...
	  {	  
			  ...
			  for (int y = -window_; y < window_ + 1; y++)
        {
          ...
          if (angle < min_angle_ || angle > max_angle_)
          {
            ...
            {
              if (copy_ptr_org[i].distance < copy_ptr_org[index].distance) // delete neighbor if they are farther away (note not self)
	...
	//除去尖峰噪点
        ...
        for(int j=i-1;j>0;j--)
        {
          if(copy_ptr_org[j].intensity <0.1 || std::isinf((double)copy_ptr_org[j].intensity) || std::isnan((double)copy_ptr_org[j].intensity) )
            {
              continue;
            }
            else
        ...
        double angle1 = abs(angles::to_degrees(getAngleWithViewpoint (copy_ptr_org[i].distance*0.002,copy_ptr_org[left_i].distance*0.002, (i - left_i) * Degree2Radians(float((copy_ptr_org[i+1].angle-copy_ptr_org[i].angle)/100.0f)))));
        double angle2 = abs(angles::to_degrees(getAngleWithViewpoint (copy_ptr_org[i].distance*0.002,copy_ptr_org[right_i].distance*0.002, (right_i - i) * Degree2Radians(float((copy_ptr_org[i+1].angle-copy_ptr_org[i].angle)/100.0f)))));
        double angle3 = angle1 + angle2;
        if(angle3>180 ) angle3 = 360 - angle3;
        double angle4=angles::to_degrees((right_i - left_i) * Degree2Radians(float((copy_ptr_org[i+1].angle-copy_ptr_org[i].angle)/100.0f)));
        if(angle3 < angle4) continue;
        angle3 = angle3 - angle4;
        if(angle3 < 70 || ((i-left_i)==1 && (right_i-i)==1))
        {
          //是尖峰
          if (copy_ptr_org[i].distance < copy_ptr_org[left_i].distance && copy_ptr_org[left_i].distance < copy_ptr_org[right_i].distance) // delete neighbor if they are farther away (note not self)
